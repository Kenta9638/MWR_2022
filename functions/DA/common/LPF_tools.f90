...
!-----------------------------------------
  SUBROUTINE LPF_CORE(org_xf,loc_fact,org_Yo,org_R,      & ! IN
    ...
    REAL(r_size),INTENT(OUT) :: out_min_res_mdl(PARA_L96_J), out_min_res_obs(PARA_obs_num)
    INTEGER     ,PARAMETER   :: max_iterations = 999 ! number of allowed iterations
    REAL(r_size)  :: max_res 
    ...
    ! Adaptive Stop
    INTEGER :: SW_samplesize
    REAL(r_size),ALLOCATABLE,DIMENSION(:)   :: SW_alpha_mdl, SW_alpha_obs
    REAL(r_size),ALLOCATABLE,DIMENSION(:)   :: tmp_roys_mdl,tmp_roys_obs
    REAL(r_size),ALLOCATABLE,DIMENSION(:)   :: tmp_pax, xd_mdl, xd_obs
    REAL(r_size),PARAMETER :: likelihood_bw   = 1.0d0
    REAL(r_size),PARAMETER :: likelihood_tail = 3.0d0
    INTEGER     ,PARAMETER :: likelihood_num  = 100
    REAL(r_size) :: SW_threshold_LoPF, SW_threshold_EnKF
    REAL(r_size),ALLOCATABLE,DIMENSION(:)   :: var_infl_mdl, var_infl_obs
    ! others
    INTEGER :: i,j,k,l,m,n
    ...
    INTEGER     ,ALLOCATABLE,DIMENSION(:)       :: tmp_i1_mtrx1, tmp_i1_mtrx2
  ...
  !!     Yo(i) = dble(i)
  !!     DO j = 1,10
  !!       hx(i,j) = dble(i)*0.20d0+dble(j)*0.90d0
  !!     ENDDO
  !!   ENDDO
  !!   DO i = 1,40
  !!     DO j = 1,10
  !!       x(i,j) = dble(i)*0.30d0+dble(j)*0.80d0
  !!     ENDDO
  !!   ENDDO


    !--- min_res ---
    ALLOCATE(min_res_mdl(PARA_L96_J)); min_res_mdl = PARA_DA_LPF_ini_minres
    ALLOCATE(min_res_obs(OBS_num));    min_res_obs = PARA_DA_LPF_ini_minres
    IF (PARA_DA_LPF_adaptive_stop) THEN
      IF (PARA_obs_nonlinear==0) THEN
        SW_threshold_LoPF = 0.05d0
        SW_threshold_EnKF = 0.35d0 ! 0.50d0
      ELSEIF (PARA_obs_nonlinear==1) THEN
        SW_threshold_LoPF = 0.05d0
        SW_threshold_EnKF = 0.05d0
      ELSEIF (PARA_obs_nonlinear==2) THEN
        SW_threshold_LoPF = 0.05d0
        SW_threshold_EnKF = 0.05d0
      ELSEIF (PARA_obs_nonlinear==91) THEN
        SW_threshold_LoPF = 0.00d0
        SW_threshold_EnKF = 0.70d0
      ...
      ELSEIF (PARA_obs_nonlinear==93) THEN
        ...
        SW_threshold_EnKF = 0.05d0
      ENDIF
      min_res_mdl = SW_threshold_EnKF
      min_res_obs = SW_threshold_EnKF
!!      IF ()
    ENDIF
      ...
      !  write(*,*) PARA_DA_LPF_adaptive_stop, PARA_DA_LPF_ini_minres
      !  print *
      !  read *
      !----------------------------------------
      ! MAIN LOOP
      ...
      DO WHILE (max_res > 0) 
        ...
        !------------------------------------------
        ! Adaptive Stop Jan. 2022
        !------------------------------------------
        IF (PARA_DA_LPF_adaptive_stop) THEN

          !- setting
          ALLOCATE(SW_alpha_mdl(PARA_L96_J))
          ALLOCATE(SW_alpha_obs(OBS_num))
          SW_alpha_mdl = 0.05d0
          SW_alpha_obs = 0.05d0
!!          SW_samplesize = PARA_DA_MEM

          ALLOCATE(tmp_roys_mdl(PARA_L96_J)); tmp_roys_mdl = UNDEF
          ALLOCATE(tmp_roys_obs(OBS_num));    tmp_roys_obs = UNDEF
   
          !- model space
          DO i = 1,PARA_L96_J
            IF (res_mdl(i)>0.0d0 .AND. res_mdl(i)<=(1.0d0-SW_threshold_EnKF-SW_threshold_LoPF)) THEN

        !!      ALLOCATE(tmp_i1_mtrx2(PARA_DA_MEM))
        !!      CALL DETECT_OUTLIERS(PARA_DA_MEM,x(i,:),tmp_i1_mtrx2)
        !!    !  tmp_i1_mtrx2 = 0
        !!      tmp_num1 = sum(tmp_i1_mtrx2)
        !!      IF (tmp_num1>0) THEN
        !!        SW_samplesize = PARA_DA_MEM - tmp_num1
        !!      ENDIF 
        !!      ALLOCATE(tmp_i1_mtrx1(SW_samplesize)); tmp_i1_mtrx1 = -9999
        !!      COUNTER1 = 0
        !!      DO j = 1,PARA_DA_MEM
        !!        IF (tmp_i1_mtrx2(j)==0) THEN ! not outlier
        !!          COUNTER1 = COUNTER1 + 1
        !!          tmp_i1_mtrx1(COUNTER1) = j
        !!        ENDIF
        !!      ENDDO
        !!      DEALLOCATE(tmp_i1_mtrx2)
        !!      ALLOCATE(tmp_d1_mtrx1(SW_samplesize))
        !!      DO j = 1,SW_samplesize
        !!        tmp_d1_mtrx1(j) = x(i,tmp_i1_mtrx1(j)) ! tmp_xf
        !!      ENDDO
              CALL SW_ROYSTON_TEST(x(i,:),PARA_DA_MEM,1,SW_alpha_mdl(i),tmp_roys_mdl(i)) ! Royston's Multivariate Normality Test
              IF (tmp_roys_mdl(i) == 1.0d0) THEN ! Gaussian (Adaptive Stop)
                out_min_res_mdl(i) = res_mdl(i)
                res_mdl(i)  = 0.0d0
              ENDIF
  !!            DEALLOCATE(tmp_d1_mtrx1,tmp_i1_mtrx1)
            ENDIF
          ENDDO
   !       write(*,*) 'tmp_roys_mdl---'
   !       write(*,*) tmp_roys_mdl
          ...
          !- obs space
          DO i = 1,OBS_num
            IF (res_obs(i)>0.0d0 .AND. res_obs(i)<=(1.0d0-SW_threshold_EnKF-SW_threshold_LoPF)) THEN

          !!!    ALLOCATE(tmp_i1_mtrx2(PARA_DA_MEM))
          !!!    CALL DETECT_OUTLIERS(PARA_DA_MEM,hx(i,:),tmp_i1_mtrx2)
          !!!   ! tmp_i1_mtrx2 = 0
          !!!    tmp_num1 = sum(tmp_i1_mtrx2)
          !!!    IF (tmp_num1>0) THEN
          !!!      SW_samplesize = PARA_DA_MEM - tmp_num1
          ...
          !!!    ALLOCATE(tmp_d1_mtrx1(SW_samplesize))
          !!!    DO j = 1,SW_samplesize
          !!!      tmp_d1_mtrx1(j) = hx(i,tmp_i1_mtrx1(j)) ! tmp_xf
          !!!    ENDDO
              CALL SW_ROYSTON_TEST(hx(i,:),PARA_DA_MEM,1,SW_alpha_obs(i),tmp_roys_obs(i))! Royston's Multivariate Normality Test
              IF (tmp_roys_obs(i) == 1.0d0) THEN ! Gaussian (Adaptive Stop)
                out_min_res_obs(i) = res_obs(i)
                res_obs(i)  = 0.0d0
              ENDIF
!!              DEALLOCATE(tmp_d1_mtrx1,tmp_i1_mtrx1)
            ENDIF
          ...
          DEALLOCATE(tmp_roys_mdl,tmp_roys_obs)
          DEALLOCATE(SW_alpha_mdl,SW_alpha_obs)
        ENDIF
...
!!          read *
    ...
    IF (PARA_DA_LPF_adaptive_stop) THEN
      DO i = 1,PARA_L96_J
        IF (out_min_res_mdl(i) < 0) THEN
          out_min_res_mdl(i) = SW_threshold_EnKF ! (=min_res_mdl)
        ELSE 
          out_min_res_mdl(i) = out_min_res_mdl(i)+SW_threshold_EnKF
        ENDIF
      ENDDO
  !    write(*,*) 'out_min_res_mdl---'
  !    write(*,*) out_min_res_mdl
  !    print *
  !    read *
      DO i = 1,OBS_num
        IF (out_min_res_obs(i) < 0) THEN
          out_min_res_obs(i) = SW_threshold_EnKF ! (=min_res_obs)
        ELSE
          out_min_res_obs(i) = out_min_res_obs(i)+SW_threshold_EnKF
        ENDIF
      ENDDO
!      write(*,*) out_min_res_mdl
!      print * 
!      read *
      ALLOCATE(var_infl_mdl(PARA_L96_J))
      ALLOCATE(var_infl_obs(OBS_num)) !; var_infl_obs = 1.0d0
      var_infl_mdl = out_min_res_mdl
      var_infl_obs = out_min_res_obs
      CALL Serial_EnSRF_CORE_tempered_v2(x,loc_fact,OBS_num,H,hx,Yo,R,var_infl_mdl,var_infl_obs,xa)
      DEALLOCATE(var_infl_mdl,var_infl_obs)
...
!    tmp_w = tmp_w/sum(tmp_w)
  ...
  SUBROUTINE DETECT_OUTLIERS(ndim,in_var,out_var)
  ! ref: isoutlier.m in MATLAB
    IMPLICIT NONE
    INTEGER     ,INTENT(IN)   :: ndim
    REAL(r_size),INTENT(IN)   :: in_var(ndim)
    INTEGER     ,INTENT(OUT)  :: out_var(ndim) ! 0 or 1 (1:outlier)
    REAL(r_size),PARAMETER :: erfcinv = -0.476936276204470d0 ! erfcinv(3/2)
    REAL(r_size) :: c, MAD
    REAL(r_size) :: tmp_median1, tmp_median2
    REAL(r_size) :: tmp_mtx1(ndim)
    INTEGER :: i

    out_var = 0
    c = -1.0d0/(sqrt(2.0d0)*erfcinv)

    CALL CAL_MEDIAN(ndim,in_var,tmp_median1)
    tmp_mtx1 = abs(in_var-tmp_median1)
    CALL CAL_MEDIAN(ndim,tmp_mtx1,tmp_median2)
    MAD = c*tmp_median2
    DO i = 1,ndim
      IF (tmp_mtx1(i)>MAD*3.0d0) THEN
        out_var(i) = 1
      ENDIF
    ENDDO
...
!!!!!!!!!-----------------------------------------------------------------------
!!!!!!!!SUBROUTINE CAL_MIN_RES_SW_TEST(dim1,xf,loc,min_res)
!!!!!!!!  IMPLICIT NONE
!!!!!!!!  !-- in & out
!!!!!!!!  INTEGER     ,INTENT(IN)  :: dim1
!!!!!!!!  REAL(r_size),INTENT(IN)  :: xf     (dim1,PARA_DA_MEM)
...
!!!!!!!!
!!!!!!!!  !--- ALLOCATE
!!!!!!!!  ALLOCATE(tmp_d2_mtrx1(2,PARA_DA_MEM)) ! tmp_xf
!!!!!!!!  ALLOCATE(tmp_d1_mtrx1(dim1))          ! tmp_loc_fact
!!!!!!!!  ALLOCATE(JPDF_ind(dim1,dim1))    
!!!!!!!!  ALLOCATE(JPDF_loc(dim1,dim1)); JPDF_loc = UNDEF   
!!!!!!!!  ALLOCATE(JPDF_dat(dim1,dim1)); JPDF_dat = UNDEF
!!!!!!!!  ALLOCATE(JPDF_cnt(dim1))    
!!!!!!!!
!!!!!!!!  !- 1. The Shapiro-Wilk test (Shapiro and Wilk, 1965) (bivariational case) -
!!!!!!!!  !$omp parallel do private(i,tmp_d2_mtrx1,tmp_d1_mtrx1,counter1,j)
!!!!!!!!  DO i = 1,dim1
!!!!!!!!    tmp_d2_mtrx1(1,:) = xf (i,:)
!!!!!!!!    tmp_d1_mtrx1      = loc(:,i) 
!!!!!!!!    counter1          = 0
!!!!!!!!    DO j = 1,dim1
!!!!!!!!      IF (tmp_d1_mtrx1(j) > 0.00001d0) THEN
!!!!!!!!        counter1 = counter1 + 1
!!!!!!!!        tmp_d2_mtrx1(2,:) = xf(j,:)
!!!!!!!!        JPDF_ind(i,counter1) = j
!!!!!!!!        JPDF_loc(i,counter1) = tmp_d1_mtrx1(j)
!!!!!!!!        JPDF_dat(i,counter1) = 0.8d0
!!!!!!!!        CALL SW_ROYSTON_TEST(tmp_d2_mtrx1,JPDF_dat(i,counter1)) ! Royston's Multivariate Normality Test
!!!!!!!!      ENDIF
!!!!!!!!    ENDDO
!!!!!!!!    JPDF_cnt(i) = counter1
!!!!!!!!!    write(*,*) i
...
!!!!!!!!  !$omp end parallel do
!!!!!!!!    
!!!!!!!!  !- 2. convert distance to min_res -
!!!!!!!!  DO i = 1,dim1
!!!!!!!!    ALLOCATE(tmp_dat(JPDF_cnt(i)))
!!!!!!!!    ALLOCATE(tmp_loc(JPDF_cnt(i)))
!!!!!!!!    tmp_dat  = JPDF_dat(i,1:JPDF_cnt(i))
!!!!!!!!    tmp_loc  = JPDF_loc(i,1:JPDF_cnt(i))
!!!!!!!!    tmp_var1 = dot_product(tmp_dat,tmp_loc)/sum(tmp_loc)
!!!!!!!!    !- convert
!!!!!!!!    min_res(i) = max(tmp_var1,0.05d0)
!!!!!!!!    min_res(i) = min(tmp_var1,0.95d0)
!!!!!!!!    DEALLOCATE(tmp_dat,tmp_loc)
!!!!!!!!  ENDDO
!!!!!!!!
!!!!!!!!  !- 3. deallocate
!!!!!!!!  DEALLOCATE(tmp_d2_mtrx1,tmp_d1_mtrx1)
!!!!!!!!  DEALLOCATE(JPDF_ind,JPDF_loc,JPDF_dat,JPDF_cnt)
!!!!!!!!
